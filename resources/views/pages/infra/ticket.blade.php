...
<section class="section">
    <div class="section-header bg-white p-4 rounded shadow-sm mb-4">
        <div>
            <h1 class="text-dark font-weight-bold" style="font-size: 1.75rem; letter-spacing: -0.5px;">IT Infrastructure Tickets</h1>
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center border-bottom pb-3 mb-3">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                @elseif($ticket->status == 'PROGRESS')
                                    <span class="badge badge-primary rounded-pill font-weight-bold text-uppercase" style="font-size: 0.75rem;">ON PROGRESS</span>
                        ...
                        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center pt-2 gap-3">
                            ...
                            <div class="action-buttons-group d-flex flex-wrap align-items-center gap-2 mt-3 mt-lg-0">
                                ...
                                @elseif ($ticket->status == 'TODO' && Auth::user()->role_id != 1)
                                    <form action="{{ route('tickets.infra.startWork', $ticket->id) }}" method="POST" class="d-inline form-start-work">
                                        @csrf
                                ...
                                @if ($ticket->created_by === Auth::id())
                                    ...
                                    <form action="{{ route('tickets.infra.delete', $ticket->id) }}" method="POST" class="d-inline form-delete-ticket">
                                        @csrf
                                    ...
                                    @if ($ticket->status == 'NEED_REVIEW')
                                        <form action="{{ route('tickets.infra.close', $ticket->id) }}" method="POST" class="d-inline form-close-ticket">
                                            @csrf
...
@foreach ($tickets as $ticket)
    @include('pages.infra.modal.detail', ['ticket' => $ticket])
    @include('pages.infra.modal.edit', ['ticket' => $ticket])
    @include('pages.infra.modal.reasonPending', ['ticket' => $ticket])
    @include('pages.infra.modal.setPending', ['ticket' => $ticket])
@endforeach
@include('pages.infra.modal.create')
